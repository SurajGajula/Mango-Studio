...
const OverlayAudioTrackComponent = ({
  ...
  return (
    <div className={styles.overlayRow} style={{ height: '32px', minHeight: '32px' }}>
      ...
      />
      {audios.filter((a) => a.isOverlay && a.row === rowIndex).map((audio) => {
        ...
        return (
          ...
          >
            <div className={styles.overlayHandleStart} onMouseDown={(e) => handleAudioTrimStart(audio.id, 'start', e)} onClick={(e) => e.stopPropagation()} />
            <div className={styles.overlayHandleEnd} onMouseDown={(e) => handleAudioTrimStart(audio.id, 'end', e)} onClick={(e) => e.stopPropagation()} />
            <div className={styles.overlayBox}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" style={{ flexShrink: 0, color: '#60a5fa' }}>
                <path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z" />
              </svg>
              <span className={styles.overlayName} style={{ color: '#bfdbfe' }}>{audio.name}</span>
            </div>
          </div>
        )
      })}
